...
def fCheckWorkflowStatus(fc_wf_id):
    ...
    while True:
        ...
        root_logger.info(
            "fCheckWorkflowStatus : Workflow Response " + r_wf_start_response.text
        )
        ...
        r_workflow_state = r_wf_tree.xpath(
            "//xip_wf:WorkflowInstance/xip_wf:State", namespaces=NSMAP
        )
...
def fCopyAllFiles(ca_target_folder):
    ...
    for ind_file in list_filepath:
        root_logger.info(
            "fCopyAllFiles : PreAmble " + str(list_reference_folder_path[0])
        )
...
def fCopyData(cd_package_source, cd_package_working, cd_sub_r):
    ...
    try:
        ...
        root_logger.info(
            ...
            + str(cd_sub_r)
            + " : Copy completed from "
            + str(cd_package_source)
            + " to "
            + str(cd_package_working)
        )
        return True
...
def fCopytreeData(cd_package_source, cd_package_working):
    ...
    try:
        ...
        root_logger.info(
            ": fCopytreeData : Copy completed from "
            + str(cd_package_source)
            + " to "
            + str(cd_package_working)
        )
        ...
        root_logger.info(
            ": fCopytreeData : Copy failed from "
            + str(cd_package_source)
            + " to "
            + str(cd_package_working)
        )
...
def fCreateContainerFolderOpexFragment(ccf_target_folder):
    ...
    root_logger.info(
        "fCreateContainerFolderOpexFragment : directory in scope " + c_folder_val
    )
    c_list_folders_in_dir.clear()
    c_list_files_in_dir.clear()
    ...
    if os.path.isdir(c_folder_val_full_path):
        ...
        for c_lfd in range(len(c_list_folders_in_dir)):
            c_opex_data_folder = (
                c_opex_data_folder
                + "<opex:Folder>"
                + c_list_folders_in_dir[c_lfd]
                + "</opex:Folder>"
            )
            root_logger.info(
                "fCreateContainerFolderOpexFragment : opex_data_folder "
                + c_opex_data_folder
            )
        for c_lff in range(len(c_list_files_in_dir)):
            if os.path.splitext(c_list_files_in_dir[c_lff])[1] == ".opex":
                c_opex_data_file = (
                    c_opex_data_file
                    + '<opex:File type="metadata">'
                    + c_list_files_in_dir[c_lff]
                    + "</opex:File>"
                )
            else:
                c_opex_data_file = (
                    c_opex_data_file
                    + '<opex:File type="content">'
                    + c_list_files_in_dir[c_lff]
                    + "</opex:File>"
                )
        source_ID = ""
        ...
        c_xml_package = fCreateOpexFragment(
            c_opex_data_folder,
            c_opex_data_file,
            opex_fixity_type,
            opex_fixity_checksum,
        ...
        try:
            ...
            c_opex_temp.close()
            root_logger.info(
                "fCreateContainerFolderOpexFragment : file created " + c_temp_opex_file
            )
        except:
            root_logger.warning(
                "fCreateContainerFolderOpexFragment : opex could not be created "
                + c_temp_opex_file
            )
        ...
        for fil in opex_f:
            ...
            if os.path.isfile(opex_file):
                ...
                if append_descriptive_metadata == 1:
                    root_logger.info(
                        "fCreateFileOpexFragments : opex_file " + str(opex_file)
                    )
                    ...
                    root_logger.info(
                        "fCreateFileOpexFragments : metadata bag ref " + str(bag_ref)
                    )
                    metadata_payload = ""
                    # metadata_payload_dc = ""
                ...
                file_xml_package = fCreateOpexFragment(
                    opex_data_folder,
                    opex_data_file,
                    opex_fixity_type,
                    opex_fixity_checksum,
                ...
                opex_filepath = os.path.join(
                    cf_target_folder, container, opex_file_withext
                )
                ...
                if not os.path.exists(opex_filepath):
                    try:
                        ...
                        opex_file.close()
                        root_logger.info(
                            "fCreateFileOpexFragments : file created " + opex_filepath
                        )
                    except:
                        root_logger.warning(
                            "fCreateFileOpexFragments : opex could not be created "
                            + opex_filepath
                        )


def fCreateFolderOpexFragments(
    cf_target_folder, security_tag, ref_title, cf_wflow_type, cf_package
):
    Ident_Biblio_Key = ""
    Identifiers_biblio = ""
    Identifiers_catalog = ""
    ...
    for fol_root, fol_dir, fol_fol in os.walk(cf_target_folder):
        for fol_d in fol_dir:
            ...
            if actual_folder_depth == 1:
                ...
                desc_metadata_xml = fCreateDigArchMetadataFragments(
                    "mdfrag1", CMSCollectionID
                )
            ...
            if os.path.isdir(opex_fol):
                ...
                for lfd in range(len(list_folders_in_dir)):
                    opex_data_folder = (
                        opex_data_folder
                        + "<opex:Folder>"
                        + list_folders_in_dir[lfd]
                        + "</opex:Folder>"
                    )
                    root_logger.info(
                        "fCreateFolderOpexFragments 4 : opex_data_folder "
                        + opex_data_folder
                    )
                for lff in range(len(list_files_in_dir)):
                    ...
                    root_logger.info(
                        "fCreateFolderOpexFragments 5 : opex_data_file "
                        + opex_data_folder
                    )
                ...
                xml_package = fCreateOpexFragment(
                    opex_data_folder,
                    opex_data_file,
                    opex_fixity_type,
                    opex_fixity_checksum,
                ...
                try:
                    ...
                    opex_temp.close()
                    root_logger.info(
                        "fCreateFolderOpexFragments 6 : file created " + temp_opex_file
                    )
                except:
                    root_logger.warning(
                        "fCreateFolderOpexFragments : opex could not be created "
                        + temp_opex_file
                    )


def fCreateOpexFragment(
    opex_data_folder,
    opex_data_file,
    opex_fixity_type,
    opex_fixity_checksum,
    ...
    if opex_fixity_type == "" or opex_fixity_checksum == "":
        ...
        opex_fixity = opex_fixity_open + opex_fixity_string + opex_fixity_close
    ...
    if opex_data_folder == "" and opex_data_file == "":
        ...
        opex_manifest = (
            opex_manifest_open + opex_folders + opex_files + opex_manifest_close
        )
    ...
    if ref_fldr_title == "":
        ...
        opex_title = "<opex:Title>" + ref_fldr_title + "</opex:Title>"

    ## Opex_Description
    if ref_fldr_desc == "":
        opex_description = ""
    else:
        opex_description = "<opex:Description>" + ref_fldr_title + "</opex:Description>"

    ## Opex Security Tag
    opex_sectag = (
        "<opex:SecurityDescriptor>" + security_tag + "</opex:SecurityDescriptor>"
    )
    ...
    opex_identifiers = opex_identifiers_biblio + opex_identifiers_catalog

    opex_properties = (
        opex_properties_open
        + opex_title
        + opex_description
        + opex_sectag
        + opex_identifiers
    ...
    if xmlschema.validate(opex_xml):
        ...
        root_logger.warning(
            "fCreateOpexFragment : Metadata validation failed for " + str(opex_master)
        )
    return new_opex_xml
...
def fDelete_Content(full_path):
    ...
    try:
        ...
        root_logger.warning(
            ": fDelete_Content : Could not delete "
            + str(full_path)
            + " contains a directory"
        )


def fCreateOpexFragment_OLD(
    opex_data_folder,
    opex_data_file,
    LegacyXIP,
    Ident_Biblio_Key,
    ...
    if not Identifiers_biblio == "":
        opex_package6b2 = (
            '<opex:Identifiers><opex:Identifier type= "SOCategory">'
            + Identifiers_biblio
            + "</opex:Identifier></opex:Identifiers>"
        )
    else:
        opex_package6b2 = ""
    ...
    if opex_desc_md != "":
        opex_desc_meta = (
            "<opex:DescriptiveMetadata>" + opex_desc_md + "</opex:DescriptiveMetadata>"
        )
    else:
        opex_desc_meta = ""
...
def fDeleteWorking_Folder(dw_workingf):
    ...
    for w_dir in os.listdir(dw_workingf):
        ...
        if os.path.isdir(full_path_w_dir):
            ...
            root_logger.info(
                "fDeleteWorking_Folder : Folder " + str(full_path_w_dir) + " deleted"
            )
...
def fDeleteZip():
    ...
    for zipper in list_unpacked_export_zips:
        if os.path.exists(zipper):
            ...
            root_logger.info(
                ": fDeleteZip : delete failed because the zip file does not exist "
                + str(zipper)
            )
            err_fDeleteZip = 1
    if err_fDeleteZip == 0:
        ...
        for wf_root, wf_dir, wf_file in os.walk(working_folder_area):
            for wf_rm in wf_dir:
                ...
                try:
                    ...
                    root_logger.warning(
                        "fSanitiseFolders : Failed on delete of "
                        + str(os.path.join(wf_root, wf_rm))
                    )
                    sanitise_complete = 1
            for wf_rmf in wf_file:
                try:
                    ...
                    root_logger.warning(
                        "fSanitiseFolders : Failed on delete of "
                        + str(os.path.join(wf_root, wf_rmf))
                    )
                    sanitise_complete = 1
        ...
        for qf in qf_names:
            ...
            if os.path.isfile(os.path.join(qroot, qf)):
                ...
                path_no_ext = (
                    bucket_prefix
                    + "/"
                    + qroot.replace(qf_target_container, "")
                    .lstrip("\\")
                    .replace("\\", "/")
                    + "/"
                    + qf
                ...
                packages = qf
                response = fUpload_file(
                    qfull_path, f_no_ext, packages, f_size, path_no_ext
                )
                if response == True:
                ...
                root_logger.info(
                    ": query_folder : File " + str(packages) + " is not a zip file"
                )
        tlf_count += 1
    ...
    for qroot, qd_names, qf_names in os.walk(qcf_target_folder):
        ...
        for qf in qf_names:
            ...
            if os.path.isfile(os.path.join(qroot, qf)):
                ...
                path_no_ext = (
                    bucket_prefix
                    + qroot.replace(qcf_parent_folder, "")
                    .lstrip("\\")
                    .replace("\\", "/")
                    + "/"
                    + qf
    ...
    with open(fs_p_f_path, "r", encoding="utf-8") as man_file:
        ...
        while True:
            ...
            if man_line == "":
                ...
                if len(man_array) != 2:
                    root_logger.warning(
                        ": delimiter is not double spaced : " + str(fs_p_f_path)
                    )
                    man_array = man_line.split(" ")
                    if len(man_array) != 2:
                        root_logger.warning(
                            ": delimiter is not single spaced : " + str(fs_p_f_path)
                        )
                        continue
                ...
                if len(man_array_line) != 3:
                    root_logger.warning(
                        ": file path length is inconsistent : " + str(fs_p_f_path)
                    )
                else:
                    try:
                        array_file_extension = man_array_line[2].split(".")
                        file_extension = array_file_extension[
                            len(array_file_extension) - 1
                        ]
                    except:
                        md_fileout.write(
                            fs_p_f_path + "||||" + man_array_line[2] + "|" + "\n"
                        )
                        root_logger.warning(
                            ": couldn't determine file extension : "
                            + str(fs_p_f_path)
                            + " : "
                            + str(man_array_line[2])
                        )
                    try:
                        package_name = ""
...
def fScanSource(p_directory):
                ...
                if p_file == "manifest-sha1.txt":
                    ...
                    for app_no_ext in list_approved_pres_extension:
                        ...
                        if p_file == app_no_ext[0]:
                            list_available_pres_extension.append(
                                (app_no_ext[0], app_no_ext[1])
                            )
    ...
    print(
        "list available pres extension sorted : " + str(list_available_pres_extension)
    )

    if len(list_available_pres_extension) == 0:
        root_logger.info(
            "fScanSource : there are no valid file formats contained within package "
            + str(list_available_pres_extension)
        )
        # app_flag = 0
        return False
    else:
        ...
        if s_p_file[0].lower() == "streaming.bin":
            ...
            if resulting_file_name == "":
                root_logger.error(
                    "fScanSource : streaming.bin file could not be acquired for "
                    + str(streaming_bin_path)
                )
        ...
        dict_PAX_asset[selected_p_file] = (
            asset_type,
            norm_p_f,
            list_corresponding_rep,
            p_f_path,
        )
        ...
        for p_file in p_f_names:
            ...
            if os.path.isfile(p_f_path):
                ...
                if file_ext in list_non_approved:
                    root_logger.warning(
                        str(sub_r)
                        + " the following file is non approved and has been deleted from the working folder "
                        + str(p_f_path)
                    )
                    os.remove(p_f_path)
    ...
    for p_root, p_d_names, p_f_names in os.walk(p_directory):
        ...
        for p_file in p_f_names:
            ...
            if os.path.isfile(p_f_path):
                ...
                root_logger.info("fScanSource : p_f_path basename " + str(array_p_file))
                root_logger.info(
                    "fScanSource : array_p_file length " + str(len(array_p_file))
                )

                # Alaska specific processing - covers only the Bags use case currently
                ######################################################################################
                print(
                    "list_approved_bitstream_type_t0 "
                    + str(list_approved_bitstream_type_t0)
                )
                file_definition = "*"

                if file_definition.lower() in list_approved_bitstream_type_t0:
                    labt_index = list_approved_bitstream_type_t0.index(
                        file_definition.lower()
                    )
                    list_corresponding_rep = list_approved_bitstream_type[labt_index][1]
                else:
                    labt_index = list_approved_bitstream_type_t0.index("other")
...
def fGetStreamingBIN(fg_streaming_bin_path):
    with open(fg_streaming_bin_path, "rb") as streaming_file:
        ...
        streaming_bin_save_location = os.path.join(
            os.path.dirname(fg_streaming_bin_path), local_file_name
        )
...
def fGet_Metadata(fg_p_f_path, md_type):
    ...
    if md_type == "DC":
        ...
        with open(fg_p_f_path, "r", encoding="utf-8") as meta_file:
            ...
            new_md_xml = lxml.etree.tostring(
                md_xml, encoding="unicode", pretty_print=True
            )
            xmlschema = lxml.etree.XMLSchema(xmlschema_doc)
            if xmlschema.validate(md_xml):
                root_logger.info(
                    "fGet_Metadata : Metadata is valid for " + str(fg_p_f_path)
                )
            else:
                root_logger.warning(
                    "fGet_Metadata : Metadata validation failed for " + str(fg_p_f_path)
                )
        ...
        try:
            ...
            for list_json_data_keys, list_json_data_vals in list_json_data.items():
                if list_json_data_keys == "asset":
                    for (
                        list_json_data_vals_keys,
                        list_json_data_vals_vals,
                    ) in list_json_data_vals.items():
                        if list_json_data_vals_keys == "referenceFilename":
                            list_json_key_val.append(
                                ["referenceFilename", list_json_data_vals_vals]
                            )
                if list_json_data_keys == "bibliographic":
                    for (
                        ...
                        list_json_key_val.append(
                            [list_json_data_vals_keys, list_json_data_vals_vals]
                        )
        except json.decoder.JSONDecodeError:
            root_logger.info(
                ": fScanJSON : Error "
                + str(fs_p_f_path)
                + " could not be loaded as a json file"
            )
...
def fCreatePAX(fc_workingf_wf_package, fc_package):
    ...
    root_logger.info(
        "fCreatePAX :  workingPAXf_container " + str(workingPAXf_container)
    )
    ...
    for dict_asset_name_keys, dict_asset_name_values in dict_asset_name.items():
        ...
        root_logger.info(
            "fCreatePAX :  dict_asset_name_keys " + str(dict_asset_name_keys)
        )
        ...
        root_logger.info(
            "fCreatePAX :  workingPAXf_wf_package_PAX "
            + str(workingPAXf_wf_package_PAX)
        )

        list_pax_zip_folders.append(workingPAXf_wf_package_PAX)
        workingPAXf_wf_package_PAX_COContainer_Rep_P = os.path.join(
            workingPAXf_wf_package_PAX, PAX_asset[2]
        )
        workingPAXf_wf_package_PAX_COContainer_Rep_P_CO = os.path.join(
            workingPAXf_wf_package_PAX_COContainer_Rep_P, PAX_asset[1]
        )
        root_logger.info(
            "fCreatePAX : workingPAXf_wf_package_PAX_COContainer_Rep_P1_CO "
            + str(workingPAXf_wf_package_PAX_COContainer_Rep_P_CO)
        )
...
def fValidateChecksums():
    ...
    for (
        dict_individual_file_checksum_keys,
        dict_individual_file_checksum_vals,
    ) in dict_individual_file_checksum.items():
        dict_file_checksum_basename = os.path.basename(
            dict_individual_file_checksum_keys
        )
        val_dict_checksum_manifest = dict_checksum_manifest.get(
            dict_file_checksum_basename, "na"
        )
        if val_dict_checksum_manifest == "na":
            root_logger.error(
                "fValidateChecksums : Checksum for file "
            ...
            dict_individual_file_checksum[dict_individual_file_checksum_keys]
            ...
            root_logger.error(
                "fValidateChecksums: Calculated checksum = "
...
def fReadFileSystem(directory):
    ...
    for root, d_names, f_names in os.walk(directory):
        for f in f_names:
            ...
            print(f)
            root_logger.info(
                "fReadFileSystem : Files found in " + str(d_names) + " called " + str(f)
            )
            rfs_full_file_path = os.path.join(root, f)
            root_logger.info(
                "fReadFileSystem : full file path found in workingPAX "
            ...
            root_logger.info(
                "fReadFileSystem : file path "
                + str(rfs_full_file_path)
                + " : length "
                + str(int_rfs_file_depth)
            )
...
def fStart_Workflow(fss_container):
    ...
    wf_start_response = requests.request(
...
def fTargetCheckSum():
    ...
    for tfp in range(len(list_target_files)):
        ...
        root_logger.info(
            ":fTargetCheckSum : Physical CheckSumType "
            + str(file_name)
            + " : "
            + str(cstype)
        )
        ...
        root_logger.info(
            ":fTargetCheckSum : Physical CheckSum Value "
            + str(file_name)
            + " : "
            + str(target_checkum_val)
        )
    return True
    ...
    for zip_package in os.listdir(zip_sourcef_wf):
        ...
        if os.path.isfile(zip_sourcef_wf_zip_package):
            if os.path.splitext(zip_sourcef_wf_zip_package)[1] == ".zip":
                ...
                print(
                    "There is a non zip file in the zip_source folder. Script exiting"
                )
                sys.exit()
        elif os.path.isdir(zip_sourcef_wf_zip_package):
            print(
                "There is a non zipped content in the zip_source folder. Script exiting"
            )
            sys.exit()
...
def fUnzipData(ue_package_source, ue_package_working_folder):
    ...
    try:
        unpack_archive(
            os.path.join(ue_package_source), os.path.join(ue_package_working_folder)
        )
        print(
            str(ue_package_source)
            + " unzipped successully to "
            + str(ue_package_working_folder)
        )
        root_logger.info(
            ": fUnzipData : "
            + str(ue_package_source)
    ...
    if aaa == 1:
        ...
        try:
            ...
            root_logger.info(
                ": fUnzipData : Unzip failed for " + str(ue_package_source)
            )
            err_fUnzipData = 1
        if os.path.isdir(wp_no_ext):
            ...
            while True:
                try:
                    ...
                    root_logger.info(
                        "fUnzipData : Subfolder unzip failed for " + str(extract_dir)
                    )
                    err_fUnzipData = 1
                else:
                    extract_dir1 = path.with_name(path.stem)
                    root_logger.info(
                        "fUnzipData : Extract Directory " + str(extract_dir1)
                    )
                    unpack_archive(str(path), str(extract_dir1), "zip")
                    list_unpacked_export_zips.append(str(path))
        else:
            root_logger.info(
                ": fUnzipData : Subfolder unzip failed for " + str(package_working)
            )
            err_fUnzipData = 1
    if err_fUnzipData == 0:
        return True
...
def fZipPAX():
    ...
    for lpzf in list_pax_zip_folders:
        ...
        target_zip_file = os.path.join(
            lpzf_dirname, str(lpzf_basename) + ".pax" + ".zip"
        )
...
def fZipContent(z_working_parent_sub_folder, z_target_parent_zip_file, z_working_dir):
    ...
    try:
        ...
        for pres_acc_subfolder in os.listdir(z_working_parent_sub_folder):
            pres_acc_subfolder_path = os.path.join(
                z_working_parent_sub_folder, pres_acc_subfolder
            )
            ...
            for root, _, Z_files in os.walk(pres_acc_subfolder_path):
                for z_file in Z_files:
                    ...
                    root_logger.info(
                        ...
                        + str(z_file)
                        + " sent to "
                        + str(z_target_parent_zip_file)
                        + " in file path "
                        + str(filePath)
                        + " with len "
                        + str(filePath[lenDirPath:])
                    )
...
def fGetPackages(fg_workflow_type):
    ...
    for package in os.listdir(sourcef_wf):
        ...
        if len(array_package_breakdown) != 3:
            root_logger.info(
                str(sub_r)
                + " : package breakdown. The package name is not consistent "
                + str(array_package_breakdown)
            )
            sys.exit()
        else:
        ...
        root_logger.info(
            sub_r + " : sourcef_wf_package_name " + str(sourcef_wf_package_name)
        )
        ...
        root_logger.info(
            sub_r + " : workingPAXf_wf_extended " + str(workingPAXf_wf_extended)
        )
        root_logger.info(
            sub_r + " : workingPAXf_wf_package " + str(workingPAXf_wf_package)
        )
        ...
        root_logger.info(
            sub_r
            + " : targetf_container_wf_package "
            + str(targetf_container_wf_package)
        )
        ...
        if not fCopytreeData(sourcef_wf_package, workingf_wf_package):
            root_logger.info(
                "fProcessPackages : fCopytreeData error : Skipping package : "
                + str(sourcef_wf_package)
            )
            continue

        if not fScanSource_ApprovedFormats(workingf_wf_package):
            root_logger.info(
                "fProcessPackages : fScanSource error : Skipping package : "
                + str(sourcef_wf_package)
            )
            continue
        ...
        fCreateFolderOpexFragments(
            targetf_container, security_tag, ref_title, fg_workflow_type, package
        )
...
def fConstructTarget(fc_targetf_container_wf_package):
    ...
    package_target_folder_content = os.path.join(
        fc_targetf_container_wf_package, opex_title_content
    )
    package_target_folder_metadata = os.path.join(
        fc_targetf_container_wf_package, opex_title_metadata
    )
    list_acpf = []
    for lcf in list_contents_folder:
        ...
        array_fc_targetf_container_wf_package = fc_targetf_container_wf_package.split(
            os.sep
        )
        for acpf in range(
            len(array_fc_targetf_container_wf_package), len(array_content_parent_folder)
        ):
            list_acpf.append(array_content_parent_folder[acpf])
        print(list_acpf)
        package_target_folder_content_file = os.path.join(
            fc_targetf_container_wf_package,
            opex_title_content,
            (os.sep).join(list_acpf),
            content_file_name,
        )
    ...
    if len(list_metadata_folder) == 0:
        ...
        for lmf in list_metadata_folder:
            ...
            package_target_folder_metadata_file = os.path.join(
                package_target_folder_metadata, metadata_file_name
            )
...
def fCreateDigArchMetadataFragments(mdfrag_type, md_value):
    ...
    if mdfrag_type == "mdfrag1":
        mdfrag_header = (
            '<cmsCollection xmlns="https://nypl.org/prsv_schemas/cmsCollection">'
        )
    ...
    new_mdfrag_xml = lxml.etree.tostring(
        mdfrag_xml, encoding="unicode", pretty_print=True
    )
...
def fUploadContent_Container():
    ...
    if select_nominated_upload_folder == 1:
        ...
        nominated_target_folder = input(
            "Enter the container path you would like to upload :: "
        )
        if nominated_target_folder == "":
            ...
            returned_target_folder = fQuery_container_folder(
                nominated_target_folder, bucket_prefix
            )
...
def fListUploadDirectory():
    ...
    print(
        "Enter ALL to upload all packages, or enter the number of the package to upload, or QUIT"
    )
    c_f_input = input()
    if c_f_input == "ALL":
        print("send all")
        ...
        for c_f_key, c_f_val in dict_containerf.items():
            if fCopytreeData(
                os.path.join(targetf, c_f_val), os.path.join(uploaddirectory, c_f_val)
            ):
        ...
        if c_f_val_from_dict == "NA":
            ...
            if fCopytreeData(
                os.path.join(targetf, c_f_val_from_dict),
                os.path.join(uploaddirectory, c_f_val_from_dict),
            ):
                if start_inc_ingest_wf == 1:
...
def fProcessOptions():
    ...
    next_step = input(
        "Select the process you would like to run. Enter 1 to Create New Container or 2 to Upload Container :: "
    )
    ...
    if next_step == "1":
        ...
        while workflow_type not in list_workflows:
            workflow_number = input(
                "Select the workflow type. Enter 1 for DigArch, 2 for xxxxxxxxxxx, 3 for xxxxxxxxxxx :: "
            )
...
manually_select_source_folder = int(
    config["VARIABLES"]["Manually_select_source_folder"]
)
...
reference_foldercount_from_file = int(
    config["VARIABLES"]["Reference_foldercount_from_file"]
)
total_reference_foldercount_from_file = int(
    config["VARIABLES"]["Total_reference_foldercount_from_file"]
)
...
reference_folder_source_ID_override = int(
    config["VARIABLES"]["Reference_folder_source_ID_override"]
)
do_not_apply_folder_source_ID = int(
    config["VARIABLES"]["Do_not_apply_folder_source_ID"]
)
...
root_logger.info(
    "reference_foldercount_from_file " + str(reference_foldercount_from_file)
)
root_logger.info(
    "total_reference_foldercount_from_file "
    + str(total_reference_foldercount_from_file)
)
...
root_logger.info(
    "reference_folder_source_ID_override " + str(reference_folder_source_ID_override)
)
...
root_logger.info(
    "manually_select_checksum_manifest " + str(manually_select_checksum_manifest)
)
