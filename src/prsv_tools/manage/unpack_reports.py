...
def fProcessPackages(source_folder: Path ,working_folder: Path, target_folder: Path):
    ...
    for packages in source_folder.iterdir():
        ...
        for pkg in packages.iterdir():
            ...
            if source_dir.is_dir():
                ...
                if source_dir.is_file():
                    package_ext = packages.suffixes()
                    package_no_ext = packages.stem()
                    if package_ext == ".zip":
                        logging.info(f" : fProcessPackages : Processing : {packages}")
                        # define output file
                        outputfile_name = f"{package_no_ext}_Info_{fTime()}.csv"
                        outputfile_path = Path(target_folder, outputfile_name)
                        xlsx_outputfile_name = f"{package_no_ext}_Info_{fTime()}.xlsx"
                        xlsx_outputfile_path = Path(target_folder, xlsx_outputfile_name)
                        ...
                        logging.info(f" : fProcessPackages : Non zipped files will not be processed : {packages}")
...
if __name__ == '__main__':
    fProcessPackages(source_folder,working_folder, target_folder)
